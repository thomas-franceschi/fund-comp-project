...
void Trainer::list_pokemon () {
	...
	for ( int i = 0; i < pokemon.size(); i++ ){
		cout << i + 1 << ". " << pokemon[i].get_name() << ": " << pokemon[i].get_hit_points() << " hp,   " << pokemon[i].get_exp() << " exp,  level:" << pokemon[i].get_level() << endl;
	}
}
...
void Trainer::encounter_pokemon ( Pokemon & battle_pokemon ) {
	...
	if (pokemon[poke_number].get_hit_points() == 0) cout << "You can't fight! Go heal your pokemon" << endl;
...
int Trainer::dead_pokemon() {
	for (int i = 0; i < pokemon.size(); i++ ) {
		if (pokemon[i].get_hit_points() > 0 ) return 1;
	}
